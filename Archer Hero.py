...
redP = Image("redpotion.jpg",game,use_alpha=False)
blueP = Image("bluepotion.jpg",game,use_alpha=False)
...
while not game.over:
    ...
    if game.score >=10:
        boss1.draw()
        boss1.moveTowards(hero,1)
        knight.moveTo(x,y)
        knight.moveTowards(hero,0)
        Earcher.moveTo(x,y)
        Earcher.moveTowards(hero,0)
        mage.moveTo(x,y)
        mage.moveTowards(hero,0)
    ...
    if game.time <= 0 and game.score >=3:
        ...
        if game.score >=5:
            ...
            fire.forward(8)
            fire.move()
            if game.score is 10 and game.time <=0:
                fire.moveTo(boss1.x,boss1.y)
    ...
    if boss1.collidedWith(arrow):
        boss1.health -= 10
        boss1health.resizeBy(-2)
        mark.visible = False
    if boss1.health <=0:
        game.over = True
